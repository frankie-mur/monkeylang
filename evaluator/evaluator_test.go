...
func TestEvalBooleanExpression(t *testing.T) {
	...
	for _, tt := range tests {
		evauluated := testEval(tt.input)
		testBooleanObject(t, evauluated, tt.expected)
	}
}
...
func TestArrayLiterals(t *testing.T) {
	...
	if len(result.Elements) != 3 {
		t.Errorf("array has wrong num elements. want=%d, got=%d",
...
func TestHashLiterals(t *testing.T) {
	...
	if len(result.Pairs) != len(expected) {
		t.Errorf("hash has wrong num pairs. want=%d, got=%d",
	...
	for expectedKey, expectedValue := range expected {
		...
		if !ok {
			t.Errorf("no pair for given key in Pairs. got=%v", result.Pairs)
			continue
		}
...
func testBooleanObject(t *testing.T, obj object.Object, expected bool) bool {
	...
	if result.Value != expected {
		t.Errorf("object has wrong value.go=%t, want=%t", result.Value, expected)
	}
	return true
}
